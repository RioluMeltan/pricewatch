    ...
    def importList() -> None: 
        ...
        for i in toCheck.splitlines(): 
            ...
            try: 
                ...
                st.toast(st.session_state.watches)
            except: 
    ...
    def listRepeater() -> None: 
        ...
        for index, watch in enumerate(st.session_state.watches): 
            ...
            with st.container(height = 350): 
                ...
                # Text-based info
                with col_2: 
                    st.markdown(f'<h4>{watch.getName()}</h4>', unsafe_allow_html = True)
                ...
                with col_3: 
                    ...
                    if st.button('Copy Price to Clipboard', key = str(index) + 'price'): 
                        MainPage.priceToClipboard(watch.getResalePrice(), watch.getCurrency())

                # Close button
                with col_4: 
                    if st.button('Remove from Watchlist', key = str(index) + 'remove'): 
